...
- **`./uninstall.sh`** (`.\uninstall.ps1`) removes the agents/commands/skills/references entries and
  ...
  `/plugin uninstall agentic-workflow`.
- **`./scripts/clean.sh`** clears the loop's local state for the project it
  drives (`$AGENTIC_WORKFLOW_DIR` or the current dir). Default wipes only the
  ...
  `.agentic-workflow.json`, and `--purge` does all three. Destructive tiers prompt
  first (skip with `-y`); `--dry-run` previews without deleting.

## Commands

- `/agentic-workflow:engineering new <idea>` · `retask <id> [note]` — interview → planless
...
- `/agentic-workflow:engineering approve [id]` — the one folder-driven gate: draft → queued
...
- `/agentic-workflow:engineering plan <id>` · `claim [id]` · `watch [trigger]` (OpenCode) ·
  `unwatch` · `recover <id>` · `stop` · `status` · `doctor [fix|config]` · `metrics [7d|30d|all] [kind]` · `init` · `kinds` —
