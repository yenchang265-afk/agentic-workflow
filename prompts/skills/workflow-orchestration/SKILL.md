...
description: The protocol for driving the agentic loop in {{hostName}} — the exact MCP tool calls and workflow-* subagent spawns each stage takes. Use when running /agentic-workflow:engineering to drive plan → build → verify → review, park at the plan gate, hold the ship gate, handle a workflow_verdict, or drive another workflow kind (e.g. pr-sitter).
---

# Driving the agentic loop ({{hostName}})

You (the **main agent**) are the driver. Unlike the OpenCode original — which runs
an autonomous background driver — {{hostName}} has no such primitive, so you drive
the stages yourself: you spawn each stage as a subagent via the **{{spawnTool}}**, and
...
`agentic-workflow:workflow-verify` — the {{spawnTool}}'s `subagent_type` for a
...
{{spawnTool}}, never the `skill` tool, even though this same turn also invokes
...
2. **Plan (queued tasks only).** `workflow_stage({stage:"plan"})`, then spawn the
   ...
   engineering) — via the {{spawnTool}} with the prompt{{modelClause}}.
...
3. **Build.** Call `mcp__agentic-workflow__workflow_stage({stage:"build"})` — it arms
   ...
   via the {{spawnTool}} with the prompt (it carries the `Worktree:` line when
   isolated){{modelClause}}. When it returns,
...
4. **Verify.** `workflow_stage({stage:"verify"})` (arms the read-only bash
   ...
   the prompt{{modelClause}}. The verify
...
6. **Terminate.** On `{done}` the server has moved the task to `in-review/`,
   ...
   short summary of the loop branch's diff, then ask with **{{askTool}}**:
