...
loop; `/agent-loop` only executes approved plans. `$ARGUMENTS` selects the mode:

- **`/agent-loop-plan new <idea>`** — turn a rough idea into a **planless draft**
...
- **`/agent-loop-plan task <id>`** — plan a task (`<id>` = filename without `.md`).
  ...
  iteration cap.
- **`/agent-loop-plan approve <id>`** — approve a planned task for execution. The
  ...
  `docs/tasks/in-progress/` (the approved queue `/agent-loop watch` claims from),
...
The flow is two-step by design: `/agent-loop-plan new <idea>` interviews you and
...
draft for confirmation. Review the draft, then run `/agent-loop-plan task <id>` to
have the relevant code read and the Implementation Plan written. Review the
plan and run `/agent-loop-plan approve <id>` — then `/agent-loop task <id>` or a
`/agent-loop watch` session executes it.
