...
## 共用工作樹的執行結束後會留在工作分支——不需要任何動作

只影響 `worktreesDir: false`（預設的 worktree 模式從來不會動到你的工作樹）。
一次執行結束後，過去會把你的工作樹檢出回原本啟動時的分支；現在則會**把你
留在 `feature/<task-id>`**——也就是你正要檢視的 diff 真正所在之處。不需要
任何設定，也沒有退出的選項。

新任務的基底不受影響：若工作樹還停在迴圈自己的分支上，切出下一個分支前
會先以儲存庫的預設分支重新定基，因此任務不會互相疊加。需要別的基底時，
先自行檢出你想要的分支即可。
