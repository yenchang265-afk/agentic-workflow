...
| `verdictChannel` | `"tool"` | 檢查類階段可以透過哪些通道記錄裁定。`"tool+block"` 會加上一條由 nonce 圍欄保護的 JSON 後備通道，供無法穩定呼叫工具的模型使用——見[在較弱的模型上執行檢查階段](#在較弱的模型上執行檢查階段)。可用 `workflows.<kind>.verdictChannel` 逐類型覆寫。 |
| `verdictRetries` | `1` | 當檢查類階段結束時完全沒有記錄任何裁定，額外**不計入迭代**的重跑次數。`1` 是長久以來的行為；`0` 代表第一次沉默就以 ERROR 停止。上限 5。 |
...
`stageModels` 讓你可以把 VERIFY 或 REVIEW 指向小型的本機模型。迴圈的裁定合約
是為「能穩定呼叫工具的模型」寫的，而較弱的模型會以三種明確的方式違反它。每一
種都有對應的開關；三者的預設值都維持今天的行為，因此強模型不會有任何改變。

**1. 它從不呼叫 `workflow_verdict`。** 裁定工具之所以是受信任的通道，正是因為
自由文字不是——階段引用自己的合約、或把 repo 內容回顯到輸出中，都絕不能翻轉
控制流。完全沒有記錄的階段會被當成通道壞掉：重跑一次（不消耗迭代），然後以可
復原的 ERROR 停止。在無法呼叫工具的模型上，那就是**每一個**檢查階段，於是迴圈
永遠無法無人值守地執行。

`verdictChannel: "tool+block"` 在不放棄那條規則的前提下加上第二條通道。迴圈會
在每次檢查階段嘗試之前鑄造一個一次性的 **nonce** 並放進該次嘗試的提示裡；階段
可以用帶著它的圍欄區塊來回覆：
...
被引用的 repo 內容不可能帶有這個 nonce——它每次嘗試都重新產生，而且只出現在那
一次嘗試的提示裡——所以不受信任文字的漏洞依然是關著的。工具仍然優先：只有在
工具完全沒有記錄時才會去讀區塊。nonce 會從執行日誌與階段產出物中清除，因此後續
階段無法重複使用它。
...
**2. 它無法在一次酬載中塞進 REVIEW 的全部五個軸。** 缺少軸的呼叫仍然會被拒絕，
但它**已經**帶到的那些軸現在會被保留，因此後續呼叫只需要補上剩下的。這一項不
需要任何設定，對每一種模型都生效。被拒絕的 FAIL，其**裁定**刻意不保留——只保留
軸——否則格式錯誤的呼叫會把該階段卡死在永久拒絕的狀態。

**3. 它送出結構描述不認識的嚴重度詞。** REVIEW 代理人被指示去呼叫
`code-review-and-quality` 技能，而該技能自己的表格教的是 Critical / Nit /
Optional / Consider / FYI。這些現在會被映射到被強制執行的 `critical` /
`important` / `suggestion`，而不是被拒絕。**無法辨識**的詞會映射到 `important`
——保守失敗，讓沒有預先規劃過的嚴重度造成阻擋，而不是悄悄消失。

把 `verdictRetries`（預設 `1`，上限 `5`）調高，可以讓不穩定的模型在階段放棄前
獲得更多不計入迭代的嘗試。這些重試永遠不會消耗建置迭代。

**這些開關買不到什麼。** 弱模型仍然得**實際去做**審查。這些開關只是讓迴圈不再
因為裁定通道壞掉而停擺；它們無法把膚淺的裁定變得深入。五個空的 PASS 軸在任何
模型上都能滿足檢查。
