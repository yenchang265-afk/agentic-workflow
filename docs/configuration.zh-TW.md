...
**含 shell 的鍵是例外，只在「使用者層級」生效**：`worktreeSetup`、`notifyCommand`、
...
- **使用者層級**——共用的使用者層級檔案（`$AGENTIC_WORKFLOW_USER_CONFIG`，
  ...
  它。跨儲存庫共用的設定（審查視角、追蹤系統）屬於這裡——而且依照上方的
  分層規則，`ado` 的目的地／憑證鍵與兩個 `bashAllowlist*` 鍵**只**在這裡
  生效；其餘鍵則由儲存庫檔案逐欄位覆寫（見上方[分層與優先順序](#layers--precedence)）。
...
管理面板的**設定分頁**會讀寫 `.agentic-workflow.json`。有五個行為值得
...
- **執行期只認使用者層級的 repo 檔案鍵，會有自己的面板。** 帶有 shell
  的設定，以及 Azure DevOps 的目的地／憑證，在合併之前就會從 repo 層
  被丟掉，所以生效檢視本來就已經排除它們；「Set here, ignored at
  runtime」會逐一列出這些鍵，讓一個悄悄沒有生效的 repo 檔案設定有個
  解釋，而不只是一團謎。
...
- **`worktreesDir`**——讓每個迴圈都在自己的 `git worktree` 中執行，
  ...
  在 `worktreesDir: false` 之下，一次跑完的執行會**把你的工作樹留在
  `feature/<task-id>`**——也就是成果所在的分支，檢視 diff、修補、推送
  都在那裡進行。下一次執行不會疊在它上面：若工作樹還停在迴圈自己的
  分支上，切出新分支前會先以儲存庫的預設分支重新定基（想要別的基底，
  就自己先 `git checkout main`）。只有在你同時設了
  `ignoreBacklog: false` 時才會有一個後果：任務移動與稽核記錄會提交在
  工作分支上，而不是你原本所在的分支。
...
- **`workflows.<kind>.planVisualization`**——布林值：設為 `true` 時，該類型
  負責寫計畫的階段（帶有 `planContract` 的那個）會被提示，在**變更的
  形狀值得畫圖時**，於 `## Implementation Plan` 裡附上一或多張
  ```mermaid`` 圖——狀態／生命週期轉移、跨兩個以上套件的流程、並行或
  鎖、資料形狀變更。由 agent 自行判斷，永不由把關點強制：`runPark`
  不會檢查圖是否存在，小型或機械式的計畫也會被告知可以略過。管理
  面板的計畫審查視圖會把該圍欄渲染成實際的圖（在沙箱 iframe 中，並附
  來源切換）；逐段的 replan 留言仍然可以錨定在它上面。

  ```jsonc
  { "workflows": { "engineering": { "planVisualization": true } } }
  ```

  和 `stageFanout` 一樣，設定值在兩個方向都會贏過清單本身的旗標——而且
  這是唯一能碰到內建類型的辦法，因為它們的清單是隨 `@agentic-workflow/core`
  套件出貨的。預設關閉；不設定就與現在完全一樣。這個值只是一個布林
  值——沒有 shell、沒有路徑——所以會像 `stageContext` 一樣從 repo 層讀取。
...
- **`AGENTIC_WORKFLOW_BASE_DIR`**——新的 `feature/<id>` worktree 的
  ...
  **每次認領時即時**（`git rev-parse --abbrev-ref HEAD`）從那裡
  ...
  兩種情況都有一個例外：若讀到的分支是迴圈自己的分支（`feature/…`），
  它**不會**被當成基底，而是改用儲存庫的預設分支——這樣即使工作樹還
  停在上一個任務的分支上，也不會把下一個任務疊上去。
