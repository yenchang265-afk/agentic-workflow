...
| `notifyEvents` | unset (= all terminal events) | Which events fire `notifyCommand` (subset of `["park","done","stop","error","stage"]`). `stage` — one ping per stage the loop fires, `AW_MESSAGE` naming the stage and iteration — is opt-in: it never fires unless listed, since a run pings four or more times under it. Not shell-bearing, so a repo may narrow — never widen — what its contributors get pinged about. |
