...
falling back. A key nothing reads is warned about at load and by
`doctor config`, with the declared key it is one typo from when there is
one (a misspelled top-level key is otherwise stripped silently; a
`workflows.<kind>` knob is flagged only when it near-misses a declared one,
because those sections carry kind-specific knobs by design). For editor
completion and type checking, point the file at the generated JSON Schema:

```json
{ "$schema": "./schema/agentic-workflow.schema.json" }
```

(`schema/agentic-workflow.schema.json` in this repo, regenerated by
`pnpm gen:schema`; runtime-only rules such as "platform `ado` requires an
`ado` section" are not expressed in it.)
