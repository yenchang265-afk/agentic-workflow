...
| 指令 | 作用 |
|---|---|
| `/agentic-workflow:engineering` | engineering 迴圈：`new`、`retask`、`approve`、`approve --all`、`replan`、`abandon`、`restore`、`remove`、`show`、`priority`、`plan`、`claim [id]`、`recover`、`kinds`、`init`、`doctor [fix\|config]`、`stop`、`status` |
