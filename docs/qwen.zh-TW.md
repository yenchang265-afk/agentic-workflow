...
| 指令 | 作用 |
|---|---|
| `/agentic-workflow:engineering` | engineering 迴圈：`new`、`retask`、`approve`、`replan`、`remove`、`plan`、`claim`、`recover`、`kinds`、`doctor`、`stop`、`status` |
...
安裝腳本會回報這個衝突，並保留最先解析到的模型，而不是靜默採用最後載入的那份
manifest。
