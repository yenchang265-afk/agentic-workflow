...
的目標，而 `:111` 把貼上整份測試輸出點名為浪費的做法。但沒有任何一個 engineering
...
1. **階段代理人不會呼叫 `context-engineering`**——而實際檢視之後，也不該讓它們
   去呼叫。該 skill 陳述的正是本計畫要機制化的準則（`SKILL.md:258`、`:111`），
   最該受惠的那些角色設定也確實從來不會載入它，但「呼叫該 skill」在三個層面上
   都是錯的解法。它是純提示詞的控制手段，而上面的前提（「不能信任它自己整理
   自己的輸入」）早已判定這種手段不足——而且它的效益與需求成反比：最缺空間的
   角色設定正是跑在小模型上、最無力執行一套程序的那些。它要花 10,769 B 去對抗
   第 2 項裡的約 74 KB，而本 repo 自己的規則是「呼叫了 skill 就要完整遵循、不得
   部分套用」，所以它並不是一個廉價的指標。而且它大部分內容在階段執行當下根本
   無法執行：frontmatter 的觸發條件是「開始一個工作階段……或設定規則檔」，而
   Level 1（撰寫規則檔）、Brain Dump、Hierarchical Summary 針對的都是那個擁有
   工作階段、負責組裝提示詞的人——階段代理人兩者皆非。

   真正可執行的部分——Level 3（`:88`，相關原始碼檔案）與 Level 4（`:105`–`:111`，
   有界的錯誤輸出）——現在已經以「規則」而非「skill 呼叫」的形式，直接內嵌進
   `prompts/agents/workflow-build/body.md` 與
   `prompts/agents/workflow-plan-author/body.md`：讀得窄，以及引用失敗的那一段
   而不是貼上整份執行輸出。規則能在小模型上存活，程序不能；而且兩個角色設定加起來
   只花 870 B，不是每一個都載入 10,769 B。

   有兩件事本項刻意不主張。它對付的是與本計畫其餘部分**不同**的缺陷：在
   `promptContext` 裡被夾限的、由驅動端組出的產出物，並不是一次過寬的 `Read`
   掃描所消耗的東西——那是代理人自己的工具端預算，任何夾限都碰不到。而且不同於
   隨它一起出貨的預算旋鈕，它沒有任何可觀測性（`promptChars` 量的是組出來的
   提示詞，不是代理人讀了什麼），所以依本計畫「可觀測性」一節自己的標準，它是
