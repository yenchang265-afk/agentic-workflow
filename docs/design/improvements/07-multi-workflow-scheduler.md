English | [繁體中文](07-multi-workflow-scheduler.zh-TW.md)

# 07 — Multi-workflow kinds on a common scheduler
...
1. **`@agentic-workflow/core`** (`packages/core/`, npm workspaces): the pure loop
   ...
   `Log`).
2. **Manifest engine**: a workflow kind is a folder `workflows/<kind>/` — `workflow.json`
   ...
   engine (`core/src/workflow/engine.ts`: `advance`/`composePrompt`/`firstStep`)
   interprets them; named TS hooks in `manifest/registry.ts` are the escape
   hatch for logic a manifest can't express. `workflows/engineering/` transcribes
...
3. **Work sources + scheduler**: `source/types.ts` defines
   ...
   gains a `workflows.<kind>` section (engineering default-on, others opt-in).
4. **PR sitter** (`workflows/pr-sitter/`): TRIAGE → FIX → VERIFY → PUBLISH over
...
- **Core is a BUILT package, not shared source.** The OpenCode plugin
  ...
  (`@agentic-workflow/core/workflow/engine`). The root `prepare` script builds core
  on `npm install`.
- **Golden parity before deletion.** The pre-manifest
  `composeArgs`/`advanceOnIdle` are frozen VERBATIM inside
  `core/src/workflow/engine.test.ts` as the oracle; the suite asserts the engine
  reproduces them byte-for-byte (prompts) and deep-equal (states/actions)
  across the whole transition table, using the real
  `workflows/engineering/` files. Only then were the legacy functions deleted.
...
| Manifest loading (`workflows/<kind>/`) | `packages/core/src/manifest/load.ts` |
| Pure engine (advance/composePrompt) | `packages/core/src/workflow/engine.ts` (+ golden parity in `engine.test.ts`) |
