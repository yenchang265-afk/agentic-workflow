[English](07-multi-loop-scheduler.md) | 繁體中文
...
目標：支援多種 agentic 迴圈類型——工程工作流程、PR sitter、未來的其他
類型——共用同一個排程器，同時不改變工程迴圈的可觀察行為。

## 已完成的工作（四個增量，每個都在綠燈狀態下落地）

1. **`@agentic-loop/core`**（`packages/core/`，npm workspaces）：純粹的
   迴圈引擎，以及每一個與 host 無關的模組（任務儲存、git + 隔離、持久化、
   裁定、指標、設定）都移進了一個建置出的套件，供兩個外掛共用。MCP 的
   分岔版本（`src/lib/`，約 18 個檔案）已被刪除；`shim.ts` 現在只是實作
   核心套件的 host 介面（`host.ts`：`Shell`、`Client`、`Log`）。
2. **清單（manifest）引擎**：一種迴圈類型就是一個資料夾
   `loops/<kind>/`——`loop.json`（zod 結構描述：階段、工作／檢查類型、
   帶有 fire/park/done/stop 效果的狀態轉換表、疊代上限、工作來源綁定、
   把關點、逐階段的 bash 白名單）加上 `stages/*.md` 提示詞範本。純粹的
   引擎（`core/src/loop/engine.ts`：`advance`/`composePrompt`/`firstStep`）
   負責解讀它們；`manifest/registry.ts` 中具名的 TS hooks 則是清單無法
   表達的邏輯的逃生艙口。`loops/engineering/` 轉錄了原本的流水線。
3. **工作來源 + 排程器**：`source/types.ts` 定義了
   ...
   `loops.<kind>` 區塊（engineering 預設開啟，其他類型須自行選擇啟用）。
4. **PR sitter**（`loops/pr-sitter/`）：作用在 `github-pr` 來源上的
...
- **核心套件是一個「已建置」的套件，而不是共用原始碼。** OpenCode 外掛
  ...
  （`@agentic-loop/core/loop/engine`）來使用它。根目錄的 `prepare` 腳本
  會在 `npm install` 時建置核心套件。
- **刪除之前先做黃金基準（golden parity）比對。** 清單引擎導入之前的
  `composeArgs`/`advanceOnIdle` 被逐字凍結在 `core/src/loop/engine.test.ts`
  中作為對照基準（oracle）；測試套件會用真正的 `loops/engineering/`
...
- **提示詞範本是分區塊的，而不是自由格式。** 一個階段提示詞檔案是以
  ...
  worktree 釘選段落）由引擎（`promptContext`）預先算好，因此一般的迴圈
  類型不需要撰寫組合用的 hook。
...
| 清單載入（`loops/<kind>/`） | `packages/core/src/manifest/load.ts` |
| 純粹引擎（advance/composePrompt） | `packages/core/src/loop/engine.ts` (+ golden parity in `engine.test.ts`) |
...
| 各類型的設定與啟用 | `packages/core/src/config.ts` (`loops`, `enabledLoopKinds`) |
| OpenCode 接線（watch/idle → pollOnce） | `src/loop/driver.ts` (`tryClaim`, `sourcesFor`, `drive` returns the terminal outcome) |
| Claude 接線（`loop_claim` → pollOnce） | `claude-plugin/mcp-server/src/server.ts` |
| 階段守衛（標記白名單） | `claude-plugin/hooks/check-stage-guard.mjs` + `runs/.stage.json` `{kind, bashAllowlist}` |
| PR-sitter 階段人設 | `.opencode/agents/loop-pr-*.md`, `claude-plugin/agents/loop-pr-*.md` |
...
- **MCP 仍維持單一運作中迴圈**；OpenCode 在共用樹（shared-tree）模式
  ...
  設計。
- **`loop_start <id>`／把關點／recover 仍然是工程類型專屬的動詞**；
  PR-sitter 的工作只會透過排程器（`watch` / `loop_claim`）送達。
