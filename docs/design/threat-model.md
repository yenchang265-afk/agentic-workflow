...
`pr-sitter` is **opt-in** (`workflows.pr-sitter.enabled: true`), so these
threats apply only to installs that turn it on — leaving it off removes them
outright. What further bounds them is that the sitter never runs on its own: it acts only
...
merge, approve, close — remains human. Operators who cannot accept T7-T10
leave the kind disabled, and can additionally control it at the credential
layer (scope or withhold the `gh`/ADO token the publish stage needs).
...
differently-shaped authority. Like the PR sitter, all three are opt-in, so
their threats apply only when `workflows.<kind>.enabled` is set (and
`review-sitter`'s authority is comment-only, which bounds T11 further).
