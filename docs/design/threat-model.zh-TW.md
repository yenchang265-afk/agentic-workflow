...
- **`verdictChannel: "tool+block"`（預設關閉）** 為「執行在無法穩定呼叫工具的
  模型上」的檢查階段加上第二條受理通道——一個圍欄化的
  ` ```workflow_verdict ` JSON 區塊。它並**未**擴大 T1：該區塊必須帶有為那一次
  階段嘗試新鑄造、且只出現在該次嘗試提示中的 nonce，因此被注入的檔案內容無法
  產生出來。只有最後一個相符的區塊算數、階段必須相符，而且工具通道絕對優先
  ——只有在完全沒有記錄到工具呼叫時才會去讀區塊。殘餘風險不變：一次成功說服
  *agent* 的注入，而 agent 本來就能呼叫工具。
- **該通道引入的殘餘風險：** nonce 是不記名憑證，因此絕不能流入任何後續階段
  讀得到的地方。它會被 `redactNonce` 從執行日誌、以及串接進下一段提示的階段
  產出物中清除，並在階段結束時清空。若有某條不在這三者涵蓋範圍內的路徑洩漏了
  nonce，後續階段就能為前面的階段偽造裁定——任何新增的階段輸出持久化出口都
  必須針對這點加上測試。
