...
# 威脅模型——agentic-workflow 迴圈

當一種工作流程類型在大致無人看管的情況下執行時可能出什麼差錯——engineering 的
PLAN → BUILD → VERIFY → REVIEW 工作流程（T1–T6）以及 PR sitter（T7–T10）——
以及哪一項控制措施能夠應對。目標讀者是在一個環境中導入
`/agentic-workflow:engineering`（或某個 sitter）的團隊，在那樣的環境裡，未經審查的
...
1. **人類輸入**——目標、計畫核准、`/agentic-workflow:<kind>` 指令。可信。
...
VERIFY/REVIEW agent 讀取的某個檔案內含 `WORKFLOW_VERIFY: PASS`，或是用文字說服
agent 該項檢查已經通過。

- **控制措施：** 裁定只透過 `workflow_verdict` 外掛工具接受，且只接受來自目前
...
- **殘餘風險：** 一次成功說服 agent 以 PASS 呼叫工具的注入。透過疊代上限、
  ...
  每軸 fan-out（`stageFanout: {"review": "axis"}`）能達成同樣的緩解，但不必付
  視角模式所索取的代價。視角模式必須關掉逐趟的軸涵蓋強制檢查——因為一個視角
  並不對應到任何一個軸——所以針對 T1 的強化其實悄悄削弱了完整性保證。fan-out
  的每一趟剛好涵蓋一個*必要*軸，因此逐趟的強制檢查是收窄而非消失，而整個階段
  的要求則改在累積後的紀錄上檢查，通過才允許階段前進。有兩項殘餘風險是刻意
  接受的：聚焦的一趟仍可能主動回報不屬於自己軸的發現（無害——合併採最差者勝，
  一個離題的 PASS 無法遮蔽另一趟的 FAIL）；以及在 Claude/Qwen host 上，pass
  迴圈是由 orchestrator 掌控的，它可能略過某次 spawn——而這正是
  `workflow_advance` 的累積涵蓋閘門要偵測的，它會以 ERROR 停下，而不是拿一場
  根本沒發生的審查去行動。
...
- **殘餘風險：** 當一個存活中的外部 watcher 持有租約時，一次性認領
  （`/agentic-workflow:<kind> claim`、MCP 伺服器的 `workflow_claim`/`workflow_start`）
...
- **控制措施：** 一道常駐的待辦異動防護（guard）、一次調和掃描
  （reconciliation sweep），以及 `workflow_doctor`——機制細節見
  [docs/workflows/engineering.md § Backlog integrity rails](../workflows/engineering.md#backlog-integrity-rails)；
...
- **控制措施：** 每一個生命週期事件（記錄計畫、核准計畫、建置開始／結束、
  ...
  於該機器的 git 身分；當 `ignoreBacklog: false` 時，待辦（backlog）的異動
  還會被提交（commit）（規劃階段的提交範圍限定於 tasks 目錄；執行階段的
  備註在共用工作樹模式下搭著分支檢查點一起走，或在 worktree 模式下依每個
  終端事件提交到主工作樹）。完整的階段輸出會落在 `<tasksDir>/runs/<id>.md`
  中，並在終止時附上一份 `## Run summary`，內含各階段耗時與裁定歷史。
- **殘餘風險：** `ignoreBacklog` 預設為 `true`，因此待辦異動的 git 提交
  歷史預設是**未開啟**的選用項——帶時間戳記的備註無論如何都會寫進任務
  檔案，但只有在 `ignoreBacklog: false` 時，它們才會經由共用的 git 歷史
  持續留存。想把提交式稽核軌跡當作 T4 控制措施的儲存庫，必須明確設定它。
  此外行為者是**設定好的** git 身分，而非**經過驗證的**身分；若需要嚴格
  的身分保證，請改為透過你的程式碼託管平台（forge）來把關核准（受保護
  分支 + 對暫存計畫進行 PR 審查）。
...
選擇性啟用的 `pr-sitter` 工作流程類型（`workflows/pr-sitter/`）新增了 engineering
迴圈刻意不具備的兩件事：它會讀取陌生人能寫入的文字，而且它會推送（push）。
這些威脅只有在設定了 `workflows.pr-sitter.enabled` 時才適用。
...
形狀的工作流程類型各有一個命名空間）記錄了哪些事情已經處理過；它是一份純粹的
...
權限。每一項威脅只有在對應的 `workflows.<kind>.enabled` 被設定時才適用。

### T11. review-sitter——權限嚴格小於 PR sitter

review sitter（`workflows/review-sitter/`）讀取的是**其他人**撰寫的 PR
...
dep sitter（`workflows/dep-sitter/`）會讀取公告文字和 changelog（不可信，與
...
main sitter（`workflows/main-sitter/`）會讀取 CI 記錄（不可信——採用與 T7
...
（approve / replan / ship）、工作流程類型（backlog kind）的編寫，以及**設定
寫入**。它是共用把關點（`workflow/gate.ts`）的第四個呼叫者，而不是第四個
...
`.agentic-workflow.json` 被**重寫**。

還有一種寫入不屬於上述任何一種：一個**規劃請求**會在
`tasksDir/queued/.requests/` 放下一個標記，不移動任何東西、也不產生 commit。
與其說它是第四件要防的事，不如說它是被限制得很死的一件事——這個標記唯一的
效果，是讓下一次認領巡查（在**另一個**行程裡）重新排序一個池子，然後把它刪掉。
它無法啟動任何階段、無法認領，也碰不到 `queued/` 以外的任務；就算被偽造，
最糟的後果也只是讓一個已經核准的任務比它的優先數字所應得的更早被規劃。
和其他地方一樣，id 在變成路徑片段之前會先經過 slug 篩檢。
...
- **控制措施：** 只綁定 `127.0.0.1`（絕不綁定 `0.0.0.0`）；拒絕 `Host`
  ...
  （preflight）就無法設定這個標頭。請求主體上限為 1 MB。任務 id 和工作流程類型代稱（slug）在觸及檔案系統之前都會經過樣式篩選，且工作流程類型的寫入
  會在 `packages/core/workflows/<kind>/` 內部進行前綴檢查。
...
- **殘餘風險：** 一個被困住的認領（來自一個已崩潰的迴圈）會被讀成「正在
  ...
  （backlog doctor）存在的理由（管理面板也把它暴露出來：`GET /api/doctor` 唯讀掃描，
  `POST /api/doctor/fix` 只會釋放**過期且未在驅動中**的認領，而在 watcher
...
`.agentic-workflow.json` 是這整個模型中授予其他一切權限的檔案——程式碼平台、
