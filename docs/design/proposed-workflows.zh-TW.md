[English](proposed-workflows.md) | 繁體中文

# 提案中的工作流程類型——一份企業工作流程目錄

這是一份**提案目錄**，不是已出貨工作的設計紀錄（那是
[`improvements/`](./improvements/README.md)）。它回答一個問題：除了已出貨的
`engineering` 和 `pr-sitter` 類型之外，還有哪些工作流程類型可以把企業環境中軟體工程師的
日常工作流程自動化？
...
- **階段圖（stage graph）**——各階段及其 `kind`（`work` 會自行完成，
  `check` 必須記錄一個 `workflow_verdict`，否則會 FAIL）、隔離方式，以及狀態轉換
...
- **設定草案**——它在 `.agentic-workflow.json` 中的 `workflows.<kind>` 區塊
...
> **狀態：已出貨** —— `packages/core/workflows/dep-sitter/`。目前的行為和設定
> 見 [`docs/sitters.md`](../sitters.md) 和
> [`docs/workflows/dep-sitter.md`](../workflows/dep-sitter.md)；以下的原始草案僅為
...
- **權限與威脅備註**：backlog-write + comment。issue 內文是典型的不可信
  輸入——T7 的注入紀律（外部文字是資料，裁定只能透過 `workflow_verdict`）
...
> **狀態：已出貨** —— `packages/core/workflows/review-sitter/`。目前的行為和
> 設定見 [`docs/sitters.md`](../sitters.md) 和
> [`docs/workflows/review-sitter.md`](../workflows/review-sitter.md)；以下的原始
...
> **狀態：已出貨** —— `packages/core/workflows/main-sitter/`。目前的行為和
> 設定見 [`docs/sitters.md`](../sitters.md) 和
> [`docs/workflows/main-sitter.md`](../workflows/main-sitter.md)；以下的原始草案
...
- **階段圖**：
  1. `gather`（**check**，隔離方式 `none`，唯讀 + 平台讀取白名單）——
     從既有的執行指標（`packages/core/src/workflow/metrics.ts`）蒐集合併、
