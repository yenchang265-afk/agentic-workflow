English | [繁體中文](proposed-workflows.zh-TW.md)

# Proposed workflow kinds — an enterprise workflow catalog
...
the shipped `engineering` and `pr-sitter` kinds, what other workflow kinds would
...
- **Config sketch** — its `workflows.<kind>` section in `.agentic-workflow.json`
...
- **Config sketch**:
  ```json
  { "workflows": { "debt-groomer": { "enabled": true, "markers": ["TODO", "FIXME", "@deprecated"], "maxTasksPerRun": 5 } } }
  ```
- **Cost**: **S**.

### dep-sitter

> **Status: SHIPPED** — `packages/core/workflows/dep-sitter/`. See
> [`docs/sitters.md`](../sitters.md) and
> [`docs/workflows/dep-sitter.md`](../workflows/dep-sitter.md) for its current
...
- **Config sketch**:
  ```json
  { "workflows": { "coverage-filler": { "enabled": true, "coverageCommand": "npm run coverage -- --json" } } }
  ```
...
- **Config sketch**:
  ```json
  { "workflows": { "issue-triager": { "enabled": true, "query": "is:open is:issue no:label", "labels": { "needsInfo": "needs-info", "duplicate": "duplicate" } } } }
  ```
- **Cost**: **M** (one new source; comment authority already modeled).

### review-sitter

> **Status: SHIPPED** — `packages/core/workflows/review-sitter/`. See
> [`docs/sitters.md`](../sitters.md) and
> [`docs/workflows/review-sitter.md`](../workflows/review-sitter.md) for its current
> behavior and config; the original sketch below is history only. v1 deltas
...
- **Config sketch**:
  ```json
  { "workflows": { "backlog-groomer": { "enabled": true, "staleAfterDays": 14 } } }
  ```
- **Cost**: **S**.
...
> **Status: SHIPPED** — `packages/core/workflows/main-sitter/`. See
> [`docs/sitters.md`](../sitters.md) and
> [`docs/workflows/main-sitter.md`](../workflows/main-sitter.md) for its current
> behavior and config; the original sketch below is history only. v1 deltas
...
- **Config sketch**:
  ```json
  { "workflows": { "release-gardener": { "enabled": true, "minMerges": 8, "cadence": "weekly", "versioning": "semver" } } }
  ```
- **Cost**: **M**.
...
- **Stage graph**:
  1. `gather` (**check**, isolation `none`, read-only + platform read
     ...
     (`packages/core/src/workflow/metrics.ts`); FAIL = nothing happened (skip
     quiet days).
...
- **Config sketch**:
  ```json
  { "workflows": { "digest-reporter": { "enabled": true, "schedule": "weekdays 08:00", "webhook": null } } }
  ```
...
- **Config sketch**:
  ```json
  { "workflows": { "alert-triager": { "enabled": true, "provider": "sentry", "filter": "is:unresolved level:error", "autoFix": false } } }
  ```
- **Cost**: **L**.
