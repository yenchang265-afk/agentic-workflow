...
分為兩層。**框架**——一個共用的核心套件、一個由清單解讀的迴圈引擎，
以及一個工作來源排程器——對工程任務或 pull request 一無所知。**迴圈
類型**（`packages/core/loops/<kind>/`）是宣告式的清單加上階段提示詞，
...
- **核心套件**——`@agentic-loop/core`（npm workspace）承載兩個外掛共用
  的一切：純粹的引擎與狀態、清單層、工作來源 + 排程器、任務儲存、git
  輔助工具 + worktree 隔離、快照、裁定處理、指標，以及設定（透過把可選
  的使用者層級 `~/.agentic-loop.json` 疊放在儲存庫的
  `.agentic-loop.json` 之下解析出來——見
...
- **清單引擎**——一種迴圈類型就是
  `packages/core/loops/<kind>/loop.json`（經 zod 驗證：帶有
  ...
  `{{#path}}…{{/path}}` 條件區塊）。`loop/engine.ts` 把它解讀成一個
...
## engineering 類型（`packages/core/loops/engineering/`）
...
（`scheduler/lease.ts`）：`/agentic-loop:<kind> watch` 會原子性地建立
...
以及它的 `loops.<kind>` 設定項；四者的安全態勢都在
[威脅模型](design/threat-model.md)中。

## Claude Code 版本（`plugins/claude/`）

相同的迴圈類型和生命週期，不同的驅動方式：Claude Code 沒有背景的
`session.idle` 驅動程式，所以主 agent 是透過一個內建的 MCP 伺服器
（`mcp__agentic-loop__loop_*` 工具）而不是 agent frontmatter 權限來
...
它是透過呼叫兩個 host 呼叫的*同一組*共用進入點（`loop/gate.ts`）來
...
它也會**編輯 `.agentic-loop.json`**，一次只編輯一個具名層——絕不
