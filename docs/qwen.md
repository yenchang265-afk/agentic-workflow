...
|---|---|
| `/agentic-workflow:engineering` | The engineering loop: `new`, `retask`, `approve`, `approve --all`, `replan`, `abandon`, `restore`, `remove`, `show`, `priority`, `plan`, `claim [id]`, `recover`, `kinds`, `init`, `doctor [fix\|config]`, `stop`, `status` |
