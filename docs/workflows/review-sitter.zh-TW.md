...
實驗性，預設關閉——要用它就得加進 `.agentic-workflow.json`。同一個區段
也用來縮小它認領的範圍：

```jsonc
{
  "workflows": {
    "review-sitter": { "enabled": true, "query": "is:open review-requested:@me" }
  }
}
```

預設的查詢字串（`is:open review-requested:@me`）可透過
`workflows.review-sitter.query` 覆寫（僅限 GitHub，與 pr-sitter 相同）。所有
...
（可用 `workflows.review-sitter.query` 覆寫，僅限 GitHub）；在 ADO 上它會認領
