...
在迭代上限之前，還有三件事會提早收束一次執行。**停滯規則**：每一次被計數的
失敗都會依其結構取指紋——未達成的驗收條件與阻擋性發現，從不包含 reason 的
文字——某個檢查階段若連續兩次以同樣方式失敗（manifest 的 `stallAfter: 2`），
迴圈就在那裡停下，而不是把第三次迭代花在一場又被要求修同樣東西的重建上；
只有 reason 的 FAIL 沒有指紋，永遠不會觸發停滯。重建的回饋區塊裡每個阻擋性
發現也都帶著穩定的 id，再次出現的會標成**重複**並註明上次出現的迭代。
**計畫缺陷分支**：當失敗的是已核准的計畫本身——某一步照寫法根本做不出來——
VERIFY 或 REVIEW 可以記錄帶 `planDefect: true` 與 reason 的 FAIL，迴圈便停下
等待 `replan <id>` 而非重建；這個旗標在 PASS 上或缺少 reason 時會被拒絕。
**檢查點篩選**：自動檢查點的 `git add -A` 會把形似機密的路徑（`.env*`、
`*.pem`、`*.key`、SSH 私鑰、憑證檔）以及超過 5 MiB 的檔案排除在掃描之外並
警告是哪些；真正屬於這次變更的路徑，以明確的 `git add <path>` 提交。
見設計 46–48。

無人值守的區段還有兩個守衛機制。**`blockedBy`**：frontmatter 列出的 id 仍在板子
上（任何非終結資料夾）的任務，每次認領 walk 都會跳過——不取 marker，改認領排在
後面的手足——並在那些 id 完成、被放棄或被移除的那一刻解除，所以懸空 id 永遠不會
卡死它；只管認領，`status` 會說明誰在等誰。**檢查重跑**：發現的檢查若非零退出，
會先再跑一次才被採信（`error` 從不重跑，`pass` 也不）；先敗後過會以 FLAKY 連同失
...
- **Doctor**（`workflow_doctor` / `/agentic-workflow:engineering doctor [fix|config]`）：
  ...
  `queued/` 的請求）、允許清單的拒絕紀錄（deny log）——被拒的 bash
  指令與能放行它的設定變更——以及迴圈的遺留物：任務已不在看板上的 worktree
  與 `feature/<id>` 分支，每條分支標明是否已合併；帶上 `fix` 時只會套用明確
  無歧義的修復——把迷途項目救回 `draft/`（稽核 + 提交）、移除清空後的迷途
  資料夾、釋放過期的孤兒認領標記、清除迷途的計畫請求、清除已回報的拒絕紀錄、
  移除孤兒 worktree（絕不 `--force`、絕不動分支——報告會為人指名
  `git branch -d`/`-D`）。重複項目永遠是人類的決定。`doctor config` 則改為回報
  有效設定：各層設定檔的路徑、執行期會忽略的 repo 層鍵值、沒有東西讀的鍵
  （有的話附上它差一個字的已宣告鍵），以及遮罩機密後、實際生效的設定內容。
