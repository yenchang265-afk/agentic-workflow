...
```
/agentic-workflow:main-sitter claim | watch [poll [interval] | cron <schedule> | idle | <interval>] | unwatch | stop | status
```

**Claude Code (MCP)**

```
/agentic-workflow:main-sitter claim | status | stop
```
...
- **`workflows.main-sitter.enabled`** — default off.
- **`workflows.main-sitter.branch`** — overrides the watched branch; unset ⇒ the
...
1. **Claim the red CI head**
   ```
   /agentic-workflow:main-sitter claim
   ```
...
2. **Check status**
   ```
   /agentic-workflow:main-sitter status
   ```
...
1. **Start the watcher**
   ```
   /agentic-workflow:main-sitter watch 10m
   ```
...
2. **Stop the watcher**
   ```
   /agentic-workflow:main-sitter stop
   ```
