...
工作要麼按需執行（`/agentic-workflow:engineering plan <id>` 規劃單一
任務，`/agentic-workflow:engineering claim` 拉取下一個項目），要麼在一個
`/agentic-workflow:engineering watch [interval]` worker session 中執行
——範圍限定於 engineering 類型——它會在每一次 idle tick 加上一個
輪詢計時器（預設 5 分鐘，例如 `/agentic-workflow:engineering watch 30s`）
...
`/agentic-workflow:engineering approve [id]`（或用
...
撰寫 + 把關（`/agentic-workflow:engineering`）：
...
- `/agentic-workflow:engineering retask <id> [note]`——重塑一份**尚未規劃**的
  ...
  後者會先被移回 `draft/`，核准因此撤銷，重塑後要再核准一次。從
  `plan-review/` 起已經有計畫，該用 `replan`
- `/agentic-workflow:engineering approve [id]`——*那個*把關動詞，統一且
  ...
  需要明確的 id
- `/agentic-workflow:engineering replan [id] [reason]`——唯一的拒絕
...
- `/agentic-workflow:engineering unwatch`——停止這個 session 認領新工作
...
- `/agentic-workflow:engineering recover <id>`——從狀態快照（或其已持久化
  的計畫）恢復一個提早停止的進行中任務——不論是崩潰/重啟、使用者
  **中斷（ESC）**，還是某個 CHECK 階段結束了這次執行——從它抵達的確切階段接續
- `/agentic-workflow:engineering waive <id> <why>`——當某個 CHECK 階段在此環境
  根本無法執行時（例如需要真實瀏覽器卻沒有顯示裝置、沙箱沒有網路連不到某個服務），
  讓已停止的執行略過它繼續往下走。它會走該階段的 pass 分支，但**不會記錄 PASS**：
  豁免本身與 `<why>` 會取代該階段的判定，出現在下一階段的提示中——因此 REVIEW 會被
  告知那些檢查是「缺席」而非「已滿足」——兩者都會寫入任務檔的稽核記錄。只有非最後
  一個 CHECK 階段可以豁免；單純 FAIL 的檢查也不該豁免——帶有問題的失敗應該重新
  build，若是計畫本身要求了此環境無法提供的東西，則改用 `replan`
...
- `/agentic-workflow:engineering stop`（別名 `abort`）——中止、清除狀態
  ...
  沒有東西可以恢復）
- `/agentic-workflow:engineering status`——印出目前的迴圈（階段、疊代
...
`/agentic-workflow:engineering` 上（`new`、`retask`、`approve`、
`replan`、`plan`、`claim`、`watch`），PR sitter 則在
`/agentic-workflow:pr-sitter` 上。
...
把外掛註冊為本機外掛檔案，這樣 `/agentic-workflow:*` 指令和內建的
