...
層級 `~/.agentic-workflow.json` 之上（儲存庫層級優先）——見
...
各個 sitter（**實驗性**——以下四個 `/agentic-workflow:<sitter>` 指令、
...
`/agentic-workflow:pr-sitter`（透過 `workflows.pr-sitter` 可選啟用）、
`/agentic-workflow:review-sitter`（`workflows.review-sitter`）、
`/agentic-workflow:dep-sitter`（`workflows.dep-sitter`），以及
