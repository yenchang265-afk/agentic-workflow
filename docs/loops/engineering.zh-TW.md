...
訊息——都來自 `packages/core/loops/engineering/loop.json`；引擎只是負責
解讀它。
...
flowchart TB
    ...
    subgraph authoring["撰寫 + 把關點 — /agentic-loop:engineering new/retask/approve · 互動式，人類參與迴圈"]
        direction TB
        new["<b>/agentic-loop:engineering new &lt;idea&gt;</b><br/>主 agent 會透過訪談與你互動（interview-me），<br/>然後由 loop-plan-author 寫下它<br/>（task-backlog-management）<br/><i>draft/ 中的無計畫草稿</i>"]
        approve{{"<b>/agentic-loop:engineering approve &lt;id&gt;</b><br/>外掛把已審查的草稿排入佇列<br/>★ 人工把關點 1 —— 任務"}}
        approveplan{{"<b>/agentic-loop:engineering approve &lt;id&gt;</b><br/>外掛驗證已暫存的計畫<br/>★ 人工把關點 2 —— 計畫<br/>（replan &lt;id&gt; &lt;why&gt; → 送回 queued/）"}}
    end
    ...
    subgraph execution["迴圈本身（THE LOOP）— /agentic-loop:engineering · 無人值守，由 session.idle 驅動"]
        direction TB
        claim["<b>/agentic-loop:engineering plan &lt;id&gt;</b> — 立即規劃一個任務（唯一的 PLAN 入口）<br/><b>/agentic-loop:engineering claim</b> — 一次性拉取（僅限可建置的任務）<br/><b>/agentic-loop:engineering watch [interval]</b> — worker session，<br/>透過原子性的 mkdir lock 認領<br/>（claim/watch 絕不會自動為 queued/ 中的任務規劃）"]
        planstage["<b>PLAN</b><br/>agent：loop-plan-author · 僅限任務檔案，於主樹（main tree）<br/>skill：planning-and-task-breakdown<br/>（相關時 + api-and-interface-design、deprecation-and-migration、<br/>documentation-and-adrs）<br/><i>就地寫入 ## Implementation Plan，<br/>然後暫存 —— 迴圈結束</i>"]
        build["<b>BUILD</b><br/>agent：loop-build · edit ✅ bash ✅<br/>skills：incremental-implementation、<br/>test-driven-development<br/>（相關時 + frontend-ui-engineering、observability-and-instrumentation、<br/>code-simplification）<br/><i>在 feature/&lt;id&gt; 分支或 worktree 上進行 TDD，<br/>每次疊代一個 commit checkpoint</i>"]
        verify["<b>VERIFY</b><br/>agent：loop-verify · edit ❌ bash：測試白名單<br/>FAIL 時的 skill：debugging-and-error-recovery<br/><i>執行測試 + 驗收標準，<br/>裁定只透過 loop_verdict 工具產生</i>"]
        review["<b>REVIEW</b><br/>agent：loop-review · edit ❌ bash：唯讀<br/>skills：code-review-and-quality<br/>（+ security-and-hardening、performance-optimization）<br/><i>五軸向 diff 審查，每個 reviewLens 各一次，<br/>取最差裁定</i>"]
    end

    ship{{"<b>/agentic-loop:engineering approve &lt;id&gt;</b><br/>你審查分支 diff<br/>★ 人工把關點 3"}}
    ...
    inreview --> ship
    ship --> completed
    build -.->|"觸及疊代上限（maxIterations）：<br/>計畫可能有問題 → 人類透過<br/>/agentic-loop:engineering replan &lt;id&gt; 送回"| queued
...
| `/agentic-loop:engineering plan\|claim\|watch\|recover\|stop\|status` | 外掛 driver（`plugins/opencode/src/loop/driver.ts`） | 生成以下三個階段 agent | — | `loop-orchestration` 協定 | 階段排序、認領、快照、執行紀錄 |
| BUILD（也是 `/build`） | driver → agent | `loop-build` | edit ✅ bash ✅ | `incremental-implementation`、`test-driven-development`（相關時 + `frontend-ui-engineering`、`observability-and-instrumentation`、`code-simplification`） | 程式碼 + 每次疊代一個 commit checkpoint |
| VERIFY（也是 `/verify`） | driver → agent | `loop-verify` | edit ❌ bash：測試執行器白名單 | `debugging-and-error-recovery`（FAIL 時） | 可信的 `loop_verdict` PASS/FAIL/ERROR |
| REVIEW（也是 `/review`） | driver → agent | `loop-review` | edit ❌ bash：唯讀 git/fs | `code-review-and-quality`（+ `security-and-hardening`、`performance-optimization`） | 每個 lens 一份可信的 `loop_verdict`，取最差裁定 |
| `/plan`（臨時） | agent | `loop-plan` | 無（唯讀） | `spec-driven-development`、`planning-and-task-breakdown` | 聊天視窗中的一份計畫——不寫入任何檔案 |
...
- **Doctor**（`loop_doctor` / `/agentic-loop:engineering doctor [fix]`）：
...
- 編寫一種新的迴圈類型：[`packages/core/loops/README.md`](../../packages/core/loops/README.md)
