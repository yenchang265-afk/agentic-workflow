...
description: The protocol for driving the agentic loop in Claude Code — the exact MCP tool calls and workflow-* subagent spawns each stage takes. Use when running /agentic-workflow:engineering to drive plan → build → verify → review, park at the plan gate, hold the ship gate, handle a workflow_verdict, or drive another workflow kind (e.g. pr-sitter).
---
...
5. **Review.** `workflow_stage({stage:"review"})`, spawn the response's `agent`
   ...
   prompt (its `model` is already pinned from the response's `model` field when present), then `workflow_advance`. PASS → `{done}`. FAIL →
...
and spawn the stage subagent again with the returned prompt (its `model` is already pinned from the response's `model` field when present). If the retry also
