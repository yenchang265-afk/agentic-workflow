...
這是 OpenCode `agentic-loop` 外掛的 Claude Code 移植版。因為 Claude Code
沒有自主的背景驅動原語，這個迴圈是**由主 agent 驅動**的：
`/agentic-loop:engineering plan <id>` / `claim` 會讓主 agent 把每個階段
...
`/plugin uninstall agentic-loop`（或拿掉 `--plugin-dir`）。儲存庫內的
...
編寫任務 + 把關（`/agentic-loop:engineering`）：

- `/agentic-loop:engineering new <idea>` —— 主 agent **一律會訪談你**
...
- `/agentic-loop:engineering approve [id]` —— *唯一*的把關動詞，統一且
  ...
  `in-review/`）——草稿一律需要明確的 id。（也以 `loop_approve` 這個
  MCP 工具的形式對外開放。）
- `/agentic-loop:engineering replan [id] [reason]` —— 唯一的拒絕動詞：
  把一份暫存的計畫（或以 id 指定、觸發上限的 `in-progress/` 任務）
  送回 `queued/`，原因會被稽核記錄。（也以 `loop_reject` 這個 MCP
  工具的形式對外開放。）

迴圈本身（`/agentic-loop:engineering`）：

- `/agentic-loop:engineering plan <id>` —— 立即為一份已核准的 `queued/`
  任務執行 PLAN 階段：它會寫入計畫、把任務暫存在 `plan-review/`，然後
  迴圈就在那裡結束（驅動的 agent 接著會透過 AskUserQuestion 就地
  提供把關選項）。從 `plan` 無法到達建置——由 `claim` 驅動建置。
- `/agentic-loop:engineering claim` —— 一次性拉取下一個已可建置的
  `in-progress/` 任務（優先權數字最小的優先；無計畫的 `queued/` 任務
  絕不會被自動規劃——請用 `plan <id>`）——是 OpenCode 上
  `/agentic-loop:engineering watch` 的拉取式對應物；這個 host 上沒有
  常駐的 watch。
- `/agentic-loop:engineering status` —— 目前執行中的迴圈，加上一份
  整體待辦彙總（單獨的 `/agentic-loop:engineering` 效果相同）。
- `/agentic-loop:engineering kinds` —— 列出各迴圈類型及其啟用狀態。
- `/agentic-loop:engineering recover <id>` —— 從狀態快照恢復一個
  被中斷的迴圈。
- `/agentic-loop:engineering doctor [fix]` —— 稽核待辦是否有結構性
  損壞（迷途的資料夾、位於所有狀態資料夾之外的任務檔案、重複的 id、
  被卡住的認領標記）；帶上 `fix` 時會套用沒有歧義的修復。
- `/agentic-loop:engineering stop`（別名 `abort`）—— 中止目前執行中的
  迴圈（未完成的工作會留在迴圈分支上）。
...
`loop_claim({kind: "<kind>"})`；這裡沒有常駐的 watch，所以 `claim`
就是拉取動作）以及 `status` · `stop`（回報／中止目前執行中的迴圈；
單獨的 `/agentic-loop:<kind>` = status）：

- `/agentic-loop:pr-sitter` —— 透過 `loops.pr-sitter` 選擇啟用。
- `/agentic-loop:review-sitter` —— 透過 `loops.review-sitter.enabled` 選擇啟用。
- `/agentic-loop:dep-sitter` —— 透過 `loops.dep-sitter.enabled` 選擇啟用。
- `/agentic-loop:main-sitter` —— 透過 `loops.main-sitter.enabled` 選擇啟用。
...
整個 engineering 生命週期現在都在 `/agentic-loop:engineering`
（`new`、`retask`、`approve`、`replan`、`plan`、`claim`）上，而 PR
sitter 在 `/agentic-loop:pr-sitter` 上。

## What's inside

- `agents/` —— `loop-plan-author`（寫入已確認的草稿；以任務模式執行
  迴圈的 PLAN 階段）、`loop-plan`（獨立的唯讀規劃器）、三個建置階段
  子 agent `loop-build` / `loop-verify` / `loop-review`、pr-sitter 的
  階段子 agent `loop-pr-triage` / `loop-pr-fix` / `loop-pr-publish`，
  以及各個選擇啟用類型的 sitter 階段子 agent：review-sitter 的
  `loop-review-fetch` / `loop-review-assess` / `loop-review-publish`、
  dep-sitter 的 `loop-dep-scan` / `loop-dep-upgrade` / `loop-dep-publish`，
  以及 main-sitter 的
  `loop-main-diagnose` / `loop-main-remedy` / `loop-main-publish`
  （共用的 `loop-verify` 在其中好幾個類型中都被重複用作 VERIFY 階段）。
- `skills/` —— `loop-orchestration`（Claude 專屬的驅動協定），加上
...
- `mcp-server/` —— `agentic-loop` MCP 伺服器
  （`mcp__agentic-loop__loop_*` 工具），重複使用原本的純狀態機，
  並移植了它的 git／待辦／持久化 IO。

## Configuration

儲存庫根目錄下可選的 `.agentic-loop.json`，疊加在使用者層級的
`~/.agentic-loop.json` 之上（逐欄位比較時儲存庫層級勝出；所有欄位
都有預設值）——完整的欄位參考見
[`docs/configuration.md`](../../docs/configuration.md)。schema 與
OpenCode 外掛相同，**只是少了** `watchIntervalMinutes`（這裡沒有 watch
模式——見下方）；`loops.<kind>.trigger` 可以被解析，但在這個
只能拉取的 host 上是無作用的（`loop_claim` 仍然是手動觸發方式）；
