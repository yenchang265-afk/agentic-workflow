...
- **`retask <id> [note]`** — reshape a planless task when the drafted goal or
  ...
  3. **Always** invoke the `interview-me` skill to reshape it, seeding it with
     ...
     through **AskUserQuestion**, on the same terms as `new` step 1.
  4. Spawn the **`workflow-task-author`** subagent (Task tool) in **`retask` mode**
     ...
     with **AskUserQuestion** "Approve `<id>` now?"; on approve call
     `mcp__agentic-workflow__workflow_approve({id})` yourself and ask a second
     **AskUserQuestion** "Plan it now?", running `workflow_start({id})` → spawn
     `workflow-plan-author` (Task tool) (its `model` is already pinned from the response's `model` field when present) → `workflow_advance`
...
- **`approve [id]`** — THE gate verb, unified and folder-driven. **Handled
  ...
  **The move is already done by the time you read this — never re-run it.**
  ...
  - **Task gate** (the task is now in `queued/`) — the hook hands the turn back
    ...
    speaking, not prose to weigh: obey it. It has you ask, with **AskUserQuestion**,
...
- **`waive <id> <why>`** — call
  ...
  names with the Task tool (its `model` is already pinned from the response's `model` field when present). It takes the waived stage's pass
