---
name: agentic-loop:review-sitter
...
- **`claim`** — one-shot pull: poll for the next PR whose review is wanted
  ...
  review-sitter manifest, `packages/core/loops/review-sitter/`). A PR whose
...
- **`watch [trigger]`** — put **this** session into review-sitter worker
  ...
  (`loops.review-sitter.trigger`, default poll); an argument overrides it for
...
- **`status`** — print the current loop (stage, iteration, watch state and
  cadence). Bare `/agentic-loop:review-sitter` (no arguments) does the same.
