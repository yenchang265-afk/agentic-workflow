---
name: agentic-workflow:main-sitter
...
- **`claim`** — one-shot pull: judge the watched branch's newest head and,
  ...
  `packages/core/workflows/main-sitter/`). A green or in-flight head claims
...
- **`watch [trigger]`** — put **this** session into main-sitter worker mode.
  ...
  (`workflows.main-sitter.trigger`, default poll); an argument overrides it for
...
- **`status`** — print the current loop (stage, iteration, watch state and
  cadence). Bare `/agentic-workflow:main-sitter` (no arguments) does the same.
