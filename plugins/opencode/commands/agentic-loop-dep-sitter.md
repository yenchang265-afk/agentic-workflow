...
- **`claim`** — one-shot pull: poll the dependency reports for the next
  ...
  `packages/core/loops/dep-sitter/`). Major bumps are never claimed — they
...
- **`watch [trigger]`** — put **this** session into dep-sitter worker mode.
  ...
  (`loops.dep-sitter.trigger`, default poll); an argument overrides it for
...
- **`status`** — print the current loop (stage, iteration, watch state and
  cadence). Bare `/agentic-loop:dep-sitter` (no arguments) does the same.
