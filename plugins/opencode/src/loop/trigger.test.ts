...
test("host config rejects an invalid cron schedule at load, names the path", () => {
  assert.throws(
    () => parseConfig({ loops: { "pr-sitter": { enabled: true, trigger: { type: "cron", schedule: "junk" } } } }),
    /loops\.pr-sitter\.trigger\.schedule/,
  )
  const ok = parseConfig({ loops: { "pr-sitter": { enabled: true, trigger: { type: "cron", schedule: "*/15 * * * *" } } } })
  assert.deepEqual(ok.loops["pr-sitter"]?.trigger, { type: "cron", schedule: "*/15 * * * *" })
})
