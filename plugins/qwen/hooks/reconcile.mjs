...
var fsClient = {
  file: {
    list: async ({ query: { path: rel, directory } }) => {
      try {
        const entries = fs3.readdirSync(path3.join(directory, rel), { withFileTypes: true });
...
var claimIds = (root, tasksDir, status) => {
  try {
    return fs3.readdirSync(path3.join(root, tasksDir, status, ".claims")).filter((n) => !n.startsWith("."));
  } catch {
    return [];
  }
};
var main = async () => {
  ...
  let truncated = false;
  const notes = [];
  const inProgress = path3.join(root, tasksDir, "in-progress");
  try {
    for (const name of fs3.readdirSync(inProgress)) {
      if (!name.endsWith(".md")) continue;
      if (Date.now() > deadline) {
        truncated = true;
        break;
      }
      const body = fs3.readFileSync(path3.join(inProgress, name), "utf8");
  ...
  let snapshots = [];
  try {
    snapshots = fs3.readdirSync(path3.join(root, tasksDir, "runs")).filter((n) => n.endsWith(".state.json")).map((n) => n.replace(/\.state\.json$/, ""));
  } catch {
  }
  ...
  const pluginRoot = process.env.AGENTIC_WORKFLOW_PLUGIN_ROOT || process.env.CLAUDE_PLUGIN_ROOT || path3.resolve(path3.dirname(fileURLToPath(import.meta.url)), "..");
  const serverBuilt = fs3.existsSync(process.env.AGENTIC_WORKFLOW_SERVER_JS || path3.join(pluginRoot, "mcp-server", "dist", "server.js"));
  const lines = [];
  if (!serverBuilt)
    lines.push(
  ...
  if (serverBuilt) {
    ...
    for (const [label, dir] of [
      ["agentic-workflow: MCP server", path3.join(pluginRoot, "mcp-server")],
      ["agentic-workflow: core", path3.resolve(pluginRoot, "..", "..", "packages", "core")]
    ]) {
      try {
