...
**Shortcuts:** once a task is in the loop's hands, the user can type
**`/agent-loop approve`** (no id) to advance the one task the loop is waiting on — a
parked plan or a finished review — and **`/agent-loop reject <why>`** to send the
single parked plan back. Approving a draft into the queue stays here (`approve
<id>`, above). Both shortcuts take an optional `<id>`, needed only to disambiguate
when more than one task awaits. The verbs above are the explicit, unambiguous form.
