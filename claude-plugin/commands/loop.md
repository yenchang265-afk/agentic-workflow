...
`/loop-plan`, before the loop; there is no free-text goal mode and no in-loop
...
- **`claim`** — call `mcp__agentic-loop__loop_claim` to pick up the next
  ...
  This is the pull equivalent of the OpenCode plugin's `/loop watch` —
...
- **anything else** (including a free-text goal) — do not run it. Explain
  that planning moved to `/loop-plan new <idea>` and show this usage.
...
— a human re-plans with `/loop-plan task <id>` and re-approves.
