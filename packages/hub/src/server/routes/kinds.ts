...
export const previewKind = async (_deps: HubDeps, req: ParsedRequest): Promise<JsonResponse> => {
  ...
  const full = composeStagePrompt(
    def,
    tpl,
    promptContext(sampleState(manifest, def.name, sample)),
    undefined,
    undefined,
...
const buildChecklist = async (deps: HubDeps, manifest: WorkflowManifest): Promise<ChecklistItem[]> => {
  ...
  // engineering is the one kind on without configuration — for it an "enable
  // it" step would sit permanently unchecked while the kind is already
  // running. Every other kind, sitters included, is opt-in.
