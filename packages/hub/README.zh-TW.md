...
agentic-loop 框架的本機管理面板：**迴圈監視器**和**視覺化迴圈
建立器**，以一個小型 web 應用程式的形式提供服務。
...
**迴圈監視器** —— 每種類型一個看板，由其清單衍生而來：把關點欄位、
帶有人工把關動作（approve／replan／ship）的任務卡片，以及帶有各階段
token 用量的執行歷史。

![帶有把關點欄位和執行歷史的迴圈監視器看板](docs/screenshots/monitor.png)

**迴圈建立器** —— 把清單狀態機畫在一個畫布上：階段、狀態轉換，以及
一個側邊面板，用來編輯與引擎執行時相同的 `LoopManifestSchema`。

![顯示 engineering 迴圈各階段與狀態轉換的迴圈建立器畫布](docs/screenshots/creator.png)

**設定（Config）** —— 一次編輯 `.agentic-loop.json` 的一個層級，
...
字元比對到的結果只有在看起來像迴圈儲存庫時（存在 `.agentic-loop.json`
或 `docs/tasks`）才會保留，因此一個裝滿無關檢出的父目錄會保持安靜。
被略過的比對結果會在啟動時列印到 stderr。

除了旗標之外，你也可以在**使用者層級**的 `~/.agentic-loop.json`
（或 `$AGENTIC_LOOP_USER_CONFIG` 所指向的檔案）中加入一個 `hub` 區塊。
它只有在沒有給出 `--dir` 時才會被使用；`--port` 仍然優先。管理面板
橫跨多個儲存庫，因此任何單一儲存庫 `.agentic-loop.json` 中的 `hub`
鍵都會被忽略：
...
localStorage 中），SSE 事件和把關點通知也都會標上儲存庫 id。迴圈
類型不是以儲存庫為範圍的——它們存在於每個儲存庫共用的核心套件中，
因此建立器分頁不受影響。

## Tabs

- **迴圈監視器**：每個已啟用的迴圈類型都有一個子分頁，每個檢視都是
  ...
  `@agentic-loop/core` 進入點執行，因此瀏覽器上的核准動作和斜線指令
...
- **迴圈建立器**：把清單狀態機畫在一個 React Flow 畫布上——
  ...
  悄悄永遠不會被觸發的區塊。
- **設定（Config）**：讀寫 `.agentic-loop.json`。它**一次編輯一個
...
slug 篩檢；迴圈類型的寫入被限制在 `packages/core/loops/<kind>/` 內，
並經過 slug 驗證和前綴檢查。
...
| 寫入動作 | 影響範圍 | 防護機制 |
|---|---|---|
| 儲存一個迴圈類型（建立器） | `packages/core/loops/<kind>/` | slug + 前綴檢查；沒有 `overwrite` 就回傳 409 |
...
| 儲存設定 | `.agentic-loop.json` 的其中一層 | `X-Hub-Client`；層級明確（絕不是合併後的檢視）；以原始 JSON 寫入，因此未知的鍵會被保留；`ado.pat` 會被遮蔽，且拒絕寫入未被 gitignore 的儲存庫檔案；除非合併後的設定通過驗證，否則會被拒絕 |
