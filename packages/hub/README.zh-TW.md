...
diff 所在的分支與異動統計、審查者的建議、收在揭露區裡的 diff 本身，以及
計畫的開頭。`GET /api/review`、`GET /api/review/:status/:id/diff`。
...
- **審查佇列**（進站畫面）：所有停在把關點上的任務，涵蓋每一個已啟用的
  ...
  所在的分支與異動統計（若完成註記有記錄的話）、REVIEW 對該次執行的
  非阻擋性建議，以及計畫的開頭。在 in-review 列上，**Diff** 揭露區會渲染
  出貨所核准的 `base...branch` diff——只在展開時才抓取，以
  `workflows.<kind>.maxDiffLines`（未設則 2000 行）截斷，並指名看其餘部分的
  `git diff` 指令。分支與 base 來自該次執行的完成註記，絕不來自請求。一份
...
- **工作流程監視器**：每個已啟用的工作流程類型都有一個子分頁，每個檢視都是
  ...
  **編輯器**：可以修改標題、type、priority、labels、acceptance 和內文，
  ...
  不該被悄悄改掉。那段註解會寫進稽核註記，下一次 PLAN 會讀到它。draft 欄的
  **+ new** 會打開同一份空表單：從看板建立一份尚無計畫的草稿
  （`POST /api/tasks/draft`），像每一次待辦寫入一樣鑄造 id 並提交，然後像
  其他任務一樣在任務關卡等待。
  ...
  **待辦醫生（backlog doctor）**——和 CLI 執行的是同一個 `workflow_doctor`
  ...
  那些*過期、無迴圈在驅動*、否則會永遠拒絕把關動作的認領標記，並移除
  迴圈的孤兒 worktree（任務已不在看板上）；重複的 id 與孤兒 `feature/<id>`
  分支則只會回報——分支絕不從按鈕刪除，那一行會指名指令。
...
| 待辦醫生修復 | `tasksDir` 下的任務檔案（救援迷途檔案、移除空的迷途資料夾、釋放**過期、無迴圈在驅動**的認領標記、清掉任務已離開 `queued/` 的規劃請求）、孤兒迴圈 worktree（`git worktree remove`，絕不 `--force`），加上一次 git commit | `X-Hub-Client`；只有在認領標記過期且沒有迴圈驅動時才會釋放；當 watch 租約仍存活時完全跳過認領釋放；迷途的規劃請求則無條件清除（它的任務已經不在，不可能有東西在驅動它）；絕不解決重複的 id；絕不刪除分支 |
