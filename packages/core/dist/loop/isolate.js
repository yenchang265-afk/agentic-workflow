...
import { addWorktree, checkoutBranch, currentBranch, ensureExcluded, isDirty, isGitRepo, pruneWorktrees, removeWorktree, worktreeForBranch, } from "./git.js";
/**
 ...
 */
export const loopId = (state) => state.task?.id ?? (slugify(state.goal.split("\n")[0] ?? "") || "goal");
/** Absolute path to a task's dedicated worktree under the configured root. Pure. */
export const worktreePathFor = (directory, worktreesDir, id) => path.resolve(directory, worktreesDir, id);
/** Run the configured worktree-setup command in a fresh worktree. Warn-and-continue. */
const runWorktreeSetup = async ($, log, config, wtPath) => {
 ...
 */
export const teardownIsolation = async ($, log, directory, state) => {
    if (!state.git)
        return;
    if (state.git.worktree) {
        if (!(await removeWorktree($, directory, state.git.worktree))) {
            await log("info", `loop: worktree ${state.git.worktree} left in place (dirty or locked) — branch ${state.git.branch} holds the committed work`);
        }
        return;
    }
