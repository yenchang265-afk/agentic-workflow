...
export const isGitRepo = async ($, cwd) => (await run($, cwd, ["rev-parse", "--is-inside-work-tree"])).ok;
...
export const isDirty = async ($, cwd) => {
 ...
 */
export const checkoutBranch = async ($, cwd, branch) => {
 ...
 */
export const commitAll = async ($, cwd, message) => {
    if (!(await run($, cwd, ["add", "-A"])).ok)
        return false;
...
export const branchExists = async ($, cwd, branch) => (await run($, cwd, ["rev-parse", "--verify", "--quiet", `refs/heads/${branch}`])).ok;
/**
 ...
 */
export const addWorktree = async ($, cwd, wtPath, branch, base) => {
...
export const listWorktrees = async ($, cwd) => {
    ...
    const flush = () => {
        if (curPath)
            entries.push({ path: curPath, branch: curBranch });
        curPath = null;
        curBranch = null;
    };
...
export const worktreeForBranch = async ($, cwd, branch) => {
 ...
 */
export const ensureExcluded = async ($, cwd, rel) => {
