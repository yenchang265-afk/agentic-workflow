...
const lookup = (ctx, path) => {
    ...
    for (const key of path.split(".")) {
        if (cur === undefined || typeof cur === "string" || typeof cur === "boolean")
            return undefined;
        cur = cur[key];
    }
    return cur;
...
export const renderSection = (tpl, ctx) => {
    ...
    return out.replace(/\{\{([\w.-]+)\}\}/g, (_, path) => asString(lookup(ctx, path)));
};
/** Split a prompt file into sections on `---` lines. */
const sections = (tpl) => tpl.split(/^[ \t]*---[ \t]*$/m);
/**
 * Render a full stage prompt: each section rendered, trimmed, empties dropped,
 * joined with a blank line.
 */
export const renderPrompt = (tpl, ctx) => sections(tpl)
