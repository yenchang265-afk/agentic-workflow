...
---
{{#checks}}Check commands the loop already ran for you, in this work tree — established fact. Do not re-run them to "confirm", and do not contradict them: a red one has already floored this stage's verdict, and arguing it down is not available to you. Cite them in your verdict's evidence alongside at least one thing you observed yourself — the files you read to judge the acceptance criteria are that evidence; a PASS whose only citations are these pre-run commands is rejected. What remains yours is judging the acceptance criteria against them.
{{checks.block}}
