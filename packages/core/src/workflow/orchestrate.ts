...
import { extractPlan, priorRunFor, replanFor } from "../task/store.js"
...
export const planEntryState = (task: Task): WorkflowState =>
  startAtPlan(taskGoal(task), taskRef(task, task.path), extractPlan(task), replanFor(task)?.reason, priorRunFor(task))
