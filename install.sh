...
configure() {
  ...
  echo "  [2] Azure DevOps (MCP server + PAT)"
  choice="$(ask "Choice" "1")"
  case "$choice" in
    2) platform="ado" ;;
    *) platform="github" ;;
  ...
  if [ "$platform" = "ado" ]; then
    ...
    add_member "\"ado\":{$ado}"
    echo
    echo "  → Azure DevOps is reached through the Azure DevOps MCP server, launched"
    echo "    with npx — nothing to install. Auth is a PAT scoped to Code (read) +"
    echo "    Pull Request (contribute)."
    ...
    echo "    The engine encodes it into the server's PERSONAL_ACCESS_TOKEN itself; you"
    echo "    never base64 anything by hand."
    echo "    For the STAGE AGENTS, the server must also be registered with your host"
    echo "    under exactly the name \"azure-devops\" — the tool names in their prompts"
    echo "    hard-code it. ./bootstrap.sh registers it for you."
