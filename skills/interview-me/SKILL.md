...
It is **mandatory** inside `/loop-plan new <idea>` — every run interviews the user before the draft is written; when the idea already states a clear goal and testable criteria, the floor is a single restate-and-confirm question, never a silent skip. The `loop-plan-author` agent has a live, responsive user via its own draft-confirmation step, and nothing is queued for execution until a human later runs `/loop-plan approve <id>`.
