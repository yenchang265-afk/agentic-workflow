...
human-invoked verbs (`recover`, `waive`, `doctor fix`) opt in; the unattended
sweeps (`claimFirst`, the startup sweep) keep the wall-clock rule, because no one
is waiting on them.

**The whole ordering lives in `claimForTakeover`, once**, and every resume verb on
both hosts calls it. It was written per-host first, and the next verb added
(`waive`) copied the SHAPE without the evidence — a bare
`claimTaskSweepingStale(task, 0)`, which sweeps the brand-new claim of a run still
inside its pre-marker setup window. A four-arm fail-closed judgement is not
something a new caller should be reproducing from the paragraph above; give it the
call, with its `verb`/`doctorHint` prose, and add arms there.
