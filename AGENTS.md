...
- `packages/hub/` — the admin hub (beta): a localhost web app (`npm run hub -- --dir <repo>`) with a loop monitor (backlog board, live gate notifications, run history, token usage) and a visual loop creator; the monitor also carries the human gate moves (approve/replan/ship) through the same `@agentic-loop/core` entry points the hosts call, but never claims work or drives a stage itself — see `packages/hub/README.md`
